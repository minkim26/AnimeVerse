...
- The app is deployed: frontend on Cloudflare Workers (auto-deploys from `main`, live at https://animeverse.minsteww26.workers.dev), backend on a GCP `e2-micro` VPS running `anime-verse-backend/compose.prod.yml` behind Caddy (live at https://animeverse-app.duckdns.org), Postgres on Supabase via its session pooler. `.github/workflows/deploy.yml` redeploys the backend automatically after a successful `CI` run on `main`. See the README's "Deployment" section for the summary and `docs/superpowers/specs/2026-08-24-production-deployment-design.md` / `docs/superpowers/plans/2026-08-24-production-deployment.md` for the full design and runbook. `ci.yml` itself still has no deploy step; `deploy.yml` is the separate workflow that does.
