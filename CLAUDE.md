...
- The app is deployed: frontend on Cloudflare Workers (auto-deploys from `main`, live at https://animeverse.minsteww26.workers.dev), backend on a GCP `e2-micro` VPS running `anime-verse-backend/compose.prod.yml` behind Caddy (live at https://animeverse-app.duckdns.org), Postgres on Supabase via its session pooler. Backend deploys are currently manual (SSH in, `git pull`, `docker compose -f compose.prod.yml up -d --build`); an auto-deploy `deploy.yml` workflow is designed but not yet built. The deployment isn't actually $0/month: GCP has charged for a VM's external IP since February 2024, so this runs ~$3.65/month, not free. See the README's "Deployment" section for the summary and `docs/superpowers/specs/2026-08-24-production-deployment-design.md` / `docs/superpowers/plans/2026-08-24-production-deployment.md` for the full design, the cost breakdown, and a documented (not yet built) serverless roadmap that would actually reach $0.
...
- Files in `public/` (`robots.txt`, `sitemap.xml`) do NOT get `%VITE_VAR%` substitution. Vite copies `public/` byte-for-byte with no templating. They still hardcode the placeholder domain `https://animeverse.example`, with an inline comment. Now that a production host exists, these need a manual domain swap to `https://animeverse.minsteww26.workers.dev`; this hasn't been done yet.
- An unset `VITE_SITE_URL` at build time leaves the literal `%VITE_SITE_URL%` string in `index.html`'s canonical/og:image/og:url/JSON-LD tags (Vite's HTML substitution doesn't fall back the way `src/lib/site.ts`'s JS-side `SITE_URL` constant does). It's set correctly on Cloudflare's production build (`https://animeverse.minsteww26.workers.dev`), just not yet in the two `public/` files above.
