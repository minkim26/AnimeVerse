...
- **No dedicated rate limiter on `POST /swipes`.** `express-rate-limit` + Redis already gates `POST /users`, `POST /users/login` (credential-stuffing risk) and `POST /avatar` (expensive image processing). `POST /swipes` is a cheap authenticated write with no comparable abuse surface, and a limiter sized for abuse-prevention would misfire on the legitimate 15-20-swipe onboarding burst a limiter tuned tighter than that would actively break the feature it's protecting. Skipped; revisit if abuse is observed.
...
- **Changing `src/components/Navbar.tsx` invalidates existing Playwright visual baselines** — Navbar renders on all four pages `e2e/visual.spec.ts` snapshots (`/`, `/login`, `/signup`, `/privacy-policy`). Per `CLAUDE.md`'s UI Change Workflow, any task touching `src/pages/**` or `src/components/**` must run the `ui-change-workflow` skill before committing, and that same change auto-fires `.github/workflows/update-e2e-snapshots.yml` on push to `main`.
