...
## Roadmap: a genuinely $0/month serverless alternative
...
AWS Lambda triggered by SQS doesn't have that failure mode: SQS message arrival is itself the invocation trigger, handled by AWS's own polling infrastructure, not the app. There's no external ping to wait for and no separate "is anyone home" check. Combined with Lambda's request-driven API side, this removes the one thing that made every other serverless option a non-starter, and it does so without ever holding a VM (and therefore never needing a billable external IP) at all.

**Cost basis, checked before writing this down:**
- **Lambda:** 1M requests + 400,000 GB-seconds/month, part of AWS's permanent Always Free tier, not the 12-month new-account trial that most AWS free tier is. Confirmed current as of this check.
- **SQS:** 1M requests/month, same permanent Always Free category as Lambda.
- **Redis replacement (rate limiting, response caching):** AWS's own ElastiCache is a 12-month trial, not a fit. Upstash Redis's free tier (256MB, 500,000 commands/month) is permanent and needs no card; this app's actual Redis usage (a handful of rate-limit checks and cache reads per request, at hobby-project traffic) sits comfortably inside that.
- **API ingress:** Lambda Function URLs are always free with no request charge; API Gateway's HTTP API type is free for the first 1M requests/month for 12 months only, then $1/million after, negligible at this traffic level either way.
- **TLS/domain:** Cloudflare (already used for the frontend) can front a Lambda Function URL or API Gateway endpoint with a CNAME, handling TLS at no cost, the same role Caddy plays today.

At this app's traffic, the recurring cost of this setup rounds to $0, with no expiring credit involved anywhere in the stack, unlike GCP's IP charge or the temporary trial currently masking it.
