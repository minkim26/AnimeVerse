...
3. **Callback/Logout/Web-Origin/CORS URLs**: `http://localhost:5173`, `http://localhost:5174` (Playwright's dedicated e2e port — see `playwright.config.ts`), and `https://animeverse.minkim26.tech` in all four URL lists (Allowed Callback URLs, Allowed Logout URLs, Allowed Web Origins, Allowed Origins (CORS)). Skipping 5174 here means the real Auth0 e2e suite fails with "redirect_uri not in the list of allowed callback URLs" the first time it runs.
...
4. Merge to `main`, with Cloudflare's automatic frontend build paused first. The Postgres migration (add `auth0Id`, drop `password`) is not a separate step you run by hand: `compose.prod.yml`'s `migrate` service runs it automatically the moment the backend deploy fires, and that deploy fires automatically once CI goes green on this push, so every existing row has `auth0Id: null` as soon as the deploy completes.
5. The frontend and backend do not deploy as one coordinated unit; they go through two independent pipelines with nothing gating one on the other. `deploy.yml` gates the backend deploy on CI passing, but Cloudflare Workers Builds deploys the frontend straight from the push, uninvolved with this repo's CI at all. Pausing Cloudflare's build before merging (step 4) is what prevents the new frontend shipping while the old backend is still live; re-enable it only once the backend deploy has actually succeeded. See the implementation plan's Task 7 Step 4 for the full runbook.
...
- **Route tests that need an authenticated request** (the existing `api/*.test.ts` files, e.g. `preferences.test.ts`, `watchlist.test.ts`) currently build a real self-issued JWT via `generateToken()`. Post-migration they mint a real, validly-signed HS256 token instead (`test/helpers.ts`'s `createTestUser`), which the real `checkJwt` middleware verifies through its actual signature/issuer/audience/expiry checks, not a mock standing in for it. `lib/auth.ts` switches `checkJwt` to HS256-via-shared-secret whenever `AUTH0_TEST_SIGNING_SECRET` is set, and back to real RS256-via-JWKS otherwise, so tests exercise the exact same verification code path production does. See the implementation plan's Task 2 Step 5.
