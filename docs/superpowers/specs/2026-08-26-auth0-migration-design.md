...
3. Bulk-import them into Auth0's database connection via `custom_password_hash: { algorithm: "bcrypt", hash: "<existing hash>" }` — Auth0 accepts bcrypt directly, so nobody's password changes. Set `"email_verified": true` on every imported row: these are real, already-established accounts, and step 6 below requires that flag to be true before it will link an imported row to its Auth0 identity. Skipping this silently breaks the migration — every returning user would get a brand-new row instead of their existing one.
